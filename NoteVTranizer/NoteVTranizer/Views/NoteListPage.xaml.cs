...
namespace NoteVTranizer.Views
{
    ...
    {
        public NoteListViewModel noteListVM;
        public NoteListPage()
        {
            InitializeComponent();
            BindingContext = noteListVM = new NoteListViewModel();
        }

        protected override async void OnAppearing()
        {
            base.OnAppearing();
            //noteListVM.LoadCommand.Execute(null);
            NoteListView.ItemsSource = noteListVM.NoteList;
        ...
        private string CreateMessage()
        {
            ...
            if ((NoteListView != null) && (NoteListView.ItemsSource != null))
            {
                sb.Append(@"<html>
                              <head>
                              </head>
                ...
                foreach (Note note in NoteListView.ItemsSource)
                {
                    if (note.UseEmail)
                    {
                        idx++;
